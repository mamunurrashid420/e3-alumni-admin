...
export function HealthSectionPage() {
  ...
  const load = async (silent = false) => {
    try {
      ...
      const res = await apiClient.getHealthSection();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mainFile) {
        await apiClient.updateHealthSection({ main_image: mainFile });
      }
      if (overlapFile) {
        await apiClient.updateHealthSection({ overlapping_image: overlapFile });
      }
      toast.success('Health section images updated');
      setMainFile(null);
      setOverlapFile(null);
      load();
    } catch (err) {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div>
        <h1 className="text-2xl xl:text-3xl font-bold">Health Section Images</h1>
        <p className="text-sm lg:text-base text-gray-600 mt-1">
          Set the two images shown in the &quot;Supporting Alumni Health &amp; Wellness&quot; section on the homepage
        </p>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between space-y-0">
          <div>
            ...
            <CardDescription>
              These are the images currently displayed in the Health section on the frontend.
            </CardDescription>
          </div>

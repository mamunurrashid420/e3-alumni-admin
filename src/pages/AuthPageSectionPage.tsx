...
export function AuthPageSectionPage() {
  ...
  const load = async (silent = false) => {
    try {
      ...
      const res = await apiClient.getAuthPage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await apiClient.updateAuthPage({ image: file });
      toast.success('Login & Registration page image updated');
      setFile(null);
      load();
    } catch (err) {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div>
        <h1 className="text-2xl xl:text-3xl font-bold">Login & Registration Page Image</h1>
        <p className="text-sm lg:text-base text-gray-600 mt-1">
          Set the background image shown on the left side of the Login and Registration pages.
        </p>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between space-y-0">
          <div>
            ...
            <CardDescription>
              This image is shown on the left panel of the Login and Registration pages.
            </CardDescription>
          </div>
            ...
            <div className="relative w-full max-w-md h-64 rounded-lg border bg-gray-50 overflow-hidden flex items-center justify-center">
              {imageUrl ? (
                <>
                  <img
                    src={imageUrl}
                    alt="Current auth page background"
                ...
                <span className="text-sm text-gray-500">No image set (default will show on frontend)</span>
              )}
            </div>
          </div>
        </CardContent>

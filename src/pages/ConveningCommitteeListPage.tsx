...
import { useConveningCommittee } from '@/hooks/useConveningCommittee';
...
export function ConveningCommitteeListPage() {
  const { items, loading, error, refetch } = useConveningCommittee();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<ConveningCommitteeMember | null>(null);
  ...
  const openEdit = (row: ConveningCommitteeMember) => {
    setEditing(row);
    setForm({
      name: row.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await apiClient.updateConveningCommitteeMember(editing.id, payload);
        toast.success('Member updated');
      } else {
        await apiClient.createConveningCommitteeMember(payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient.deleteConveningCommitteeMember(deleteId);
